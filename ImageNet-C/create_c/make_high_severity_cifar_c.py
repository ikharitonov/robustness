...
def gaussian_noise(x, severity=1):
    c = [0.04, 0.06, .08, .09, .10, 0.11, 0.12, 0.14, 0.16, 0.18][severity - 1]
...
def shot_noise(x, severity=1):
    c = [500, 250, 100, 75, 50, 40, 30, 15, 10, 5][severity - 1]
...
def impulse_noise(x, severity=1):
    c = [.01, .02, .03, .05, .07, 0.08, 0.09, 0.10, 0.12, 0.14][severity - 1]
...
def speckle_noise(x, severity=1):
    c = [.06, .1, .12, .16, .2, 0.22, 0.24, 0.28, 0.3, 0.32][severity - 1]
...
def glass_blur(x, severity=1):
    ...
    c = [(0.05,1,1), (0.25,1,1), (0.4,1,1), (0.25,1,2), (0.4,1,2), (0.05,1,2), (0.25,1,2), (0.4,1,2), (0.25,1,3), (0.4,1,3)][severity - 1]
...
def defocus_blur(x, severity=1):
    c = [(0.3, 0.4), (0.4, 0.5), (0.5, 0.6), (1, 0.2), (1.5, 0.1), (1.6, 0.7), (1.7, 0.8), (1.8, 0.9), (2, 0.1), (2.5, 0.05)][severity - 1]
...
def motion_blur(x, severity=1):
    c = [(6,1), (6,1.5), (6,2), (8,2), (9,2.5), (10,3), (11,3), (12,3.5), (13,4), (14,4.5)][severity - 1]
...
def zoom_blur(x, severity=1):
    c = [np.arange(1, 1.06, 0.01), np.arange(1, 1.11, 0.01), np.arange(1, 1.16, 0.01),
         np.arange(1, 1.21, 0.01), np.arange(1, 1.26, 0.01), np.arange(1, 1.31, 0.01), np.arange(1, 1.36, 0.01), np.arange(1, 1.41, 0.01),
         np.arange(1, 1.46, 0.01), np.arange(1, 1.51, 0.01)][severity - 1]
...
def fog(x, severity=1):
    c = [(.2,3), (.5,3), (0.75,2.5), (1,2), (1.5,1.75), (1.75,1.5), (2,1.25), (2.25,1), (2.5,0.75), (3,0.5)][severity - 1]
...
def snow(x, severity=1):
    c = [(0.1,0.2,1,0.6,8,3,0.95),
         ...
         (0.3,0.3,1.25,0.7,12,10,0.75),
         (0.35,0.3,1.74,0.6,14,12,0.7),
         (0.4,0.3,1.5,0.65,14,14,0.65),
         (0.45,0.4,2.5,0.7,15,14,0.6),
         (0.5,0.4,2.75,0.75,16,14,0.55)][severity - 1]
...
def spatter(x, severity=1):
    c = [(0.62,0.1,0.7,0.7,0.5,0),
         ...
         (0.66,0.2,0.8,0.67,0.5,0),
         (0.67,0.2,0.9,0.66,0.5,0),
         (0.68,0.2,1.1,0.65,0.5,0),
         (0.68,0.2,0.8,0.64,0.6,1),
         (0.68,0.2,0.6,0.63,0.6,1)][severity - 1]
    x = np.array(x, dtype=np.float32) / 255.
...
def contrast(x, severity=1):
    c = [.75, .5, .4, .3, 0.15, 0.12, 0.1, 0.08, 0.04, 0.02][severity - 1]
...
def brightness(x, severity=1):
    c = [.05, .1, .15, .2, .3, .35, .4, .45, .5, .55][severity - 1]
...
def saturate(x, severity=1):
    c = [(0.3, 0), (0.1, 0), (1.5, 0), (2, 0.1), (2.5, 0.2), (3, 0.2), (3.5, 0.2), (4, 0.3), (4.5, 0.4), (5, 0.5)][severity - 1]
...
def pixelate(x, severity=1):
    c = [0.95, 0.9, 0.85, 0.75, 0.65, 0.55, 0.45, 0.35, 0.25, 0.15][severity - 1]
...
def elastic_transform(image, severity=1):
    ...
    c = [(IMSIZE*0, IMSIZE*0, IMSIZE*0.08),
         ...
         (IMSIZE*0.12, IMSIZE*0.03, IMSIZE*0.03),
         (IMSIZE*0.12, IMSIZE*0.02, IMSIZE*0.025),
         (IMSIZE*0.16, IMSIZE*0.015, IMSIZE*0.02),
         (IMSIZE*0.16, IMSIZE*0.012, IMSIZE*0.015),
         (IMSIZE*0.18, IMSIZE*0.01, IMSIZE*0.01)][severity - 1]
...
for method_name in d.keys():
    ...
    np.save(save_path + d[method_name].__name__ + '.npy',
            np.array(cifar_c).astype(np.uint8))

    np.save(save_path + 'labels.npy',
